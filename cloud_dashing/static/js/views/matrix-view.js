define(['backbone', 'handlebars', 'collections/agents', 'text!templates/matrix.hbs',
    'common', 'collections/detail-reports'], 
        function (Backbone, handlebars, agents, matrixTemplate, common, 
            DetailReports) {
                ...
                _renderTable: function () {
                    if (!!this._reports && !!this._reports.models) {
                        ...
                        var cloudMap = {};
                        agents.forEach(function (agent) {
                            cloudMap[agent.get('id')] = agent;
                        });
                        if (!!current_report) {
                            this.$('tbody').empty();
                            current_report.get('data').forEach(function (subReport) {
                                var cloud = cloudMap[subReport.id];
                                if (!cloud.get('selected')) {
                                    return;
                                }
                                var s = '<tr data-id="' + subReport.id + '">';
                                s += '<td>' + subReport.id + '</td>';
                                s += '<td>' + cloud.get('name') + '</td>';
