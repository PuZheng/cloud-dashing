    ...
    var DailyReport = Backbone.Model.extend({
        ...
        parse: function (response, parse) {
            ...
            response.data.forEach(function (cloudDetail) {
                ...
                if (!cloudDetail.hasOwnProperty('存储性能')) {
                    console.log('日报没有提供存储性能');
                    cloudDetail['存储性能'] = {crash_num: 0};
                }
                if (!cloudDetail.hasOwnProperty('网络性能')) {
                    console.log('日报没有提供网络性能');
                    cloudDetail['网络性能'] = {crash_num: 0};
                }
                that.crashNumMap[cloudDetail.id] = cloudDetail['计算性能']['crash_num']+ cloudDetail['存储性能']['crash_num'] + (cloudDetail['网络性能']['crash_num'] || 0);
            });
            return response;
        },
